...
string  findBinaryNotation(int num)
{
  ...
  if(num ==0)
   ...
   num /= 2;
   holder = holder % 2+num;
   binary = ((char)(holder-num+48));
   return findBinaryNotation(num) + binary;
  }
}
...
bool isPalindrome(string palin)
{
  ...
  {
	if (!(palin.substr(0,1).compare(palin.substr(palin.length()-1,1))))
  	{
	  return isPalindrome(palin.substr(1, palin.length() - 2));
	}
	else
	{
	  return false;
...
void arrRev(char (&arr)[26], int low, int high)
{
 ...
 if (low < high)
 {
  arr[low] = arr[high];
  arr[high] = holder;
  return arrRev(arr, low+1, high-1);
 }
}
